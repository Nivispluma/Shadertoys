...
const int LIGHT_COUNT = 2; // Number of lights in the scene
...
//vec3 getColor(Sphere[SPHERE_COUNT] spheres, vec3 rayOrigin, vec3 rayDirection, Camera camera, Light[LIGHT_COUNT] light){
vec3 getColor(Sphere[SPHERE_COUNT] spheres, vec3 rayOrigin, vec3 rayDirection, Camera camera, Light light){
    ...
    for (int depth = 0; depth < DEPTH; depth++){
        ...
        //for (int number_of_lights = 0; number_of_lights < LIGHT_COUNT; number_of_lights++){
            if (rayCollided){
                ...
                //vec3 fromObjectToLightVector = normalize(light[number_of_lights].position - collisionPoint);
                ...
                if (depth == 0){
                    //color = spheres[hitSphereIndex].color*light[number_of_lights].color*(
                    ...
                    //color += reflectiveness*spheres[hitSphereIndex].color*light[number_of_lights].color*(
                        spheres[hitSphereIndex].ambientStrength + diffuseStrength + spheres[hitSphereIndex].specularStrength*specular
                    );
                    reflectiveness*= spheres[hitSphereIndex].reflectiveness;
                }
...
vec3 render(vec2 fragCoord, float windowWidth, float windowHeight, Sphere[SPHERE_COUNT] spheres, Camera camera, Light[LIGHT_COUNT] light){
    vec3 color = vec3(0.0);
#ifdef ANTIALIASING
    for(float i = -1.0; i <= 1.0; i++) {
        for(float j = -1.0; j <= 1.0; j++) {
            ...
            for (int number_of_lights = 0; number_of_lights < LIGHT_COUNT; number_of_lights++){
    		    color += getColor(spheres, camera.position, rayDirection, camera, light[number_of_lights]);
            }
        }
    }
    color /= 9.0;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    Light lights[LIGHT_COUNT] = Light[LIGHT_COUNT](
        Light(
            ...
            vec3(0.8,0.5,0.2)   // intensität?
        ),
        Light(
            vec3(0.0,1.0,2.0), // position?
            vec3(0.5,0.6,0.1)   // intensität?
        )
    );


    fragColor = vec4(render(fragCoord, iResolution.x, iResolution.y, spheres, camera, lights),1.0);
}